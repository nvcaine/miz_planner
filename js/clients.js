$( function() {
	$('input[name=new-client-birthday]').datepicker({format: 'yyyy-mm-dd', autoclose: true});
	...
	$('#edit-client-popup').on('show.bs.modal', function(event) {
		if(event.relatedTarget !== undefined) {
			...
			$(this).find('input[name=edit-client-address]').val(button.data('client_address'));
			$(this).find('input[name=edit-client-phone]').val(button.data('client_phone'));
			$(this).find('input[name=edit-client-email]').val(button.data('client_email'));
			$('input[name=edit-client-birthday]').datepicker({format: 'yyyy-mm-dd', initialDate: button.data('client_birthday'), autoclose: true})
		}
	});
});